...
@ApiResponses(value = { @ApiResponse(responseCode = "400", description = "Parametros invalidos", content = {
		...
		@ApiResponse(responseCode = "404", description = "Datos no encontrados", content = {
				@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseErrorModel.class)) }),
		@ApiResponse(responseCode = "401", description = "Acceso no Autorizado", content = {
				@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseErrorModel.class)) }),
		@ApiResponse(responseCode = "503", description = "Error no controlado", content = {
				@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseErrorModel.class)) }) })
public class WeatherController {
	...
	@ApiResponse(responseCode = "200", description = "Informacion Climatologica encontrada", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = WeatherPostResponseDTO.class)) })
	public ResponseEntity<?> postWeather(@Valid @NotNull @RequestBody WeatherPostRequestDTO requestBody)
			throws JsonMappingException, JsonProcessingException, BadRequestException, ServiceUnavailableException {
	...
	@ApiResponse(responseCode = "200", description = "Historial", content = {
			@Content(mediaType = "application/json", schema = @Schema(implementation = WeatherHistory.class)) })
	public ResponseEntity<?> getWeather() {
		List<WeatherHistory> lsHistory = weatherService.getAllWeather();
		if (ObjectUtils.isEmpty(lsHistory))
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseErrorModel(
					HttpStatus.NOT_FOUND.toString(), new String[] { "No se han encontrado datos." }));
		else
